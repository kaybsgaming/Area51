...
(function ($) {
  ...
  $.fn.tagsManager = function (options, tagToManipulate) {
    ...
    var tagManagerOptions = {
      ...
      validator: null,
      onlyTagList: false,
	  onAddTag: function() { /*console.log('onAddTag(): Not implemented');*/ },
		afterRemoveTag: function() { /*console.log('afterRemoveTag(): Not implemented');*/ }
    };
    ...
    var tagBaseClass = 'btn btn-default ';
    var inputBaseClass = '';
    ...
    var tagClasses = function () {
      ...
      if (obj.attr('class')) {
        $.each(obj.attr('class').split(' '), function (index, value) {
          if (value.indexOf(inputBaseClass + '-') != -1 && !value == 'form-control') {
...
//<button aria-hidden="true" data-dismiss="modal" class="close tm-tag-remove" type="button">x</button>
 ...
 */
        var html = '<span class="' + tagClasses() + '" id="' + newTagId + '">';
        html += '<span>' + escaped + '</span>';
        html += '&nbsp;<a href="#" class="close tm-tag-remove" type="button" id="' + newTagRemoveId + '" TagIdToRemove="' + tagId + '"><sup>';
        html += tagManagerOptions.tagCloseIcon + '</sup></a></span> ';
        ...
        $el.find("#" + newTagRemoveId).on("click", obj, function (e) {
		  ...
		  tagManagerOptions.afterRemoveTag();
        });

        refreshHiddenTagList();
		
		//Finally call the tag-add event, including the text of the tag added
		tagManagerOptions.onAddTag(tag);
